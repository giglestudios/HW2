  ...
  if (numused) {
    ...
    // Remember that light positions must be transformed by modelview.
      ...
      for (int iter = 0; iter < numLights; iter++) {
          ...
          vec4 transformHolder;
          
          for (int i = 0; i < 4; i++) {
              transformHolder[i] = lightposn[offset + i];
          }
          // Light transform = Light position * Current Matrix
          transformHolder = mv * transformHolder;
          
          for (int i = 0; i < 4; i++) {
              lightransf[offset + i] = transformHolder[i];
          }
      }
  ...
  for (int i = 0 ; i < numobjects ; i++) {
    object* obj = &(objects[i]); // Grabs an object struct.
      ...
      //{
          ...
          mat4 objectTransform = transf * glm::transpose(obj->transform);
          glLoadMatrixf(&(objectTransform)[0][0]);
